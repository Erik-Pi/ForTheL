...
Theorem. Let f be a map. Let id{A} and f be composable. Then id{A}*f = f.
Proof.
 For all x << Dom{f) : (id{A}*f)(x) = id{A}(f(x)) = f(x).
Qed.

Theorem. Let f be a map. Let Dom{f) = A. Then f*id{A} = f.
Proof.
 For all x << A : (f*id{A})(x) = f(id{A}(x)) = f(x).
Qed.

Theorem CompFromTo. Let A,B,C be sets. Let g:A->B. Let f:B->C. Then f*g : A->C.

Theorem. Let A,B,C,D be sets. Let h:A->B. Let g:B->C. Let f:C->D. Then (f*g)*h : A->D.

Theorem. Let A,B,C,D be sets. Let h:A->B. Let g:B->C. Let f:C->D. Then f*(g*h) : A->D.

Theorem ThreeComp. Let A,B,C,D be sets. Let h:A->B. Let g:B->C. Let f:C->D. Then (f*g)*h = f*(g*h).
Proof.
 For all x << Dom{(f*g)*h) : ((f*g)*h)(x) = (f*(g*h))(x).
Qed.
...
Proof.
 Take a map i such that i = V2ddV(K,V).
 ...
 Let us show that i is linear over K from V to ddV.
  ...
  Let us show that for all u,v < V : i(u +{V} v) = i(u) +{ddV} i(v).
   ...
   Let us show that for all f < dual(K,V) : i(u +{V} v)(f) = (i(u) +{ddV} i(v))(f).
    ...
    qed.
   qed.
   Dom{i(u +{V} v)) = |dual(K,V)|.
   Dom{i(u) +{ddV} i(v)) = |dual(K,V)|.
 ...
 Let us show that R is a ring.
  ...
  Let us show that for all f,g,h < R : f *{R} (g +{R} h) = (f *{R} g) +{R} (f *{R} h).
   ...
   Let us show that for all v < V : (f *{R} (g +{R} h))(v) = ((f *{R} g) +{R} (f *{R} h))(v).
    ...
    = ((f *{R} g) +{R} (f *{R} h))(v).
   qed.
   Dom{f *{R} (g +{R} h)) = |V| = Dom{(f *{R} g) +{R} (f *{R} h)).
  qed.
...
Theorem. Let V be a vector space over K. Let W be a structure. Assume |V|=|W|.
...
# It takes only 05:35 to check instead of approximately 100 minutes.
